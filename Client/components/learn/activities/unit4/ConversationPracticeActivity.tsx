...
export default function GreetingsConversationPracticeActivity({
  activity,
  onComplete,
}: Props) {
  useLessonProgressReporter();
  const { award } = useCompletionXP(15, "greetings-convo");
  ...
  if (!(merged as any)?.conversation?.length) {
    return (
  ...
  return (
    <BaseConversationPracticeActivity
      ...
      onComplete={async () => {
        await award({
          screen: "GreetingsConversationPracticeActivity",
          reason: "completed",
        });
        onComplete();
      }}
...
export const componentKey = "greetings-conversation-practice";
