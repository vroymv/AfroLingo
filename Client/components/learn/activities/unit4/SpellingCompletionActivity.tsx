...
export default function GreetingsSpellingCompletionActivity({
  activity,
  onComplete,
}: Props) {
  useLessonProgressReporter();
  const { award } = useCompletionXP(15, "greetings-spelling");
  ...
  const merged = useMemo(() => {
    return {
      ...
      items: (content as any)?.items || (activity as any)?.items,
    };
  }, [activity, content]);
  ...
  return (
    <BaseSpellingCompletionActivity
      ...
      mistakeReporting={{
        ...
        screen: "GreetingsSpellingCompletionActivity",
        metadata: {
...
export const componentKey = "greetings-spelling-completion";
